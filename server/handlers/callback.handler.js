...
async function showBalance(bot, chatId, userId) {
  ...
  await bot.sendMessage(chatId, "Balance view coming soon.");
}

async function showStats(bot, chatId) {
  // TODO: Fetch group stats from contributions table
  await bot.sendMessage(chatId, "Group stats coming soon.");
}

async function handleCallbackQuery(bot, ctx, db) {
  const query = ctx.callbackQuery || ctx;
  ...
  if (data === "contribute") {
    ...
    await bot.sendMessage(chatId, "Contribution cancelled.");
